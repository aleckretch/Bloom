...
final class AddCardViewController: UIViewController {
    ...
    @objc func onTapButtonSaveCard() {
        if let cell = saveCell {
            ...
            if let cardExpirationString = cardCell?.expirationDateTextField.text, let firstName = cardCell?.firstNameTextField.text, let lastName = cardCell?.lastNameTextField.text, let phoneNumber = billingCell?.phoneNumberTextField.text, let emailAddress = billingCell?.emailAddressTextField.text, let addressLine1 = billingCell?.line1TextField.text, let addressLine2 = billingCell?.line2TextField.text,  let addressLine3 = billingCell?.line3TextField.text, let addressCity = billingCell?.cityTextField.text, let addressState = billingCell?.stateTextField.text, let addressZip = billingCell?.zipTextField.text, let cardNumber = cardCell?.numberTextField.text {
